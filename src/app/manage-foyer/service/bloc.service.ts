...
export class BlocService {
  Chambre: any;
  private baseUrl = environment.url+"/bloc";
  ...
  getChambresByFoyerAndBloc(idFoyer: number, idBloc: number): Observable<any[]> {
    return this.http.get<any[]>(`${environment.url}/chambre/foyer/${idFoyer}/${idBloc}`);
  }

  ajouterBlocAuFoyer(idFoyer: number, bloc: Bloc): Observable<void> {
    const url = `${environment.url}/foyer/${idFoyer}/ajouterBloc`;
